        ...
        private void txtmoney_TextChanged(object sender, EventArgs e) {
            if (price == "") {
                ...
                if (txtmoney.Text == "") {
                    ...
                    lbl0.Text = Convert
                        .ToDecimal(Convert.ToDouble(txtmoney.Text.Trim()) -
                                   Convert.ToDouble(price) * Convert.ToDouble(0.95) - Convert.ToDouble(bjf))
                        .ToString("C");
                }
            }
        }
        ...
        private void btnJZ_Click(object sender, EventArgs e) {
            if (txtmoney.Text == "" || lbl0.Text == "0") {
                ...
                if (lbl0.Text.Substring(1, 1) == "-") {
                    ...
                    cmd.ExecuteNonQuery();
                    cmd = new SqlCommand(
                        "update tb_Room set RoomZT='待用',Num=0,WaiterName='' where RoomName='" + Rname + "'", conn);
                    cmd.ExecuteNonQuery();
                    conn.Close();
                    this.Close();
                }
